...
var controller = Leap.loop({enableGestures: true}, function(frame) {
  frame.hands.forEach(function(hand, index) {
    ...
    var npVerticalY = [normalizedPosition[0]*testDimension.width, (1-normalizedPosition[1])*testDimension.height];
    //var npHorizonalY = [normalizedPosition[0]*testDimension.width, normalizedPosition[2]*testDimension.height];
    mouseMoved(npVerticalY[0], npVerticalY[1]);

    if (frame.gestures.length > 0) {
      var gesture = frame.gestures[0];
      switch (gesture.type) {
      //  case "circle":
      //      fittsTest.mouseClicked(np[0], np[1]);
      //      break;
        case "keyTap":
            fittsTest.mouseClicked(npVerticalY[0], npVerticalY[1]);
            break;
      //  case "screenTap":
      //      fittsTest.mouseClicked(np[0], np[1]);
      //      break;
